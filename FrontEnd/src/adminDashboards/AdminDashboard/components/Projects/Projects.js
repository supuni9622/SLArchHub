...
function Projects() {
    ...
    useEffect(() => { 
        axios.get("http://localhost:5000/admin-dashboard/projects")
            .then(res => {               
                setData(res.data)
             })
             .catch(error=>{
    ...
    return (
      <MaterialTable
        ...
        columns={[
          ...
          { title: 'Sustainability', field: 'sustainability'},
          { title: 'Floor Area (sqft)', field: 'area'},
          { title: 'Total Budget (Rs)', field: 'budget'},
          { title: 'Exterior Designer', field: 'architect' },
          ...
          { title: 'Started Date', field: 'startedDate'},
          { title: 'Finished Date', field: 'finishedDate'}             
        ]}
        data={data}
